...
def phone_numbers(inputs):
    return re.findall(r"\(\d{3}\)\s\d{3}-\d{4}", inputs)


def emails(inputs):
    return re.findall(r"[A-Za-z0-9.]+@\w+.\w+", inputs)

inputs1 = """Dear Mr. Davis,
...
print(phone_numbers(inputs1))

inputs2 = """Veggies es bonus vobis, proinde vos postulo essum magis kohlrabi
